import { Group, Period, Step, StepType } from './enums';

export const STEP_MAP: Readonly<Record<Step, string>> = {
  [Step.报名]: '报名流程',
  [Step.笔试]: '笔试流程',
  [Step.组面时间选择]: '组面时间选择流程',
  [Step.组面]: '组面流程',
  [Step.熬测]: '熬测流程',
  [Step.群面时间选择]: '群面时间选择流程',
  [Step.群面]: '群面流程',
  [Step.通过]: '通过',
};

export const RECRUITMENT_NAME_MAP: Readonly<Record<string, string>> = {
  S: '春季招新',
  C: '夏令营招新',
  A: '秋季招新',
};

export const ID_GROUP_MAP: Readonly<Record<number, Group>> = {
  2: Group.web,
  3: Group.pm,
  4: Group.ai,
  5: Group.android,
  6: Group.game,
  7: Group.lab,
  8: Group.ios,
  9: Group.design,
  15: Group.web,
  16: Group.android,
  17: Group.ai,
  18: Group.game,
  19: Group.ios,
  20: Group.design,
  21: Group.lab,
  22: Group.lab,
  23: Group.lab,
  24: Group.lab,
  25: Group.pm,
};

export const SLOTS: Readonly<Record<Period, number[]>> = {
  [Period.morning]: [9, 9.5, 10, 10.5, 11, 11.5, 8.5, 8, 7.5],
  [Period.afternoon]: [14.5, 15, 15.5, 16, 16.5, 17, 14, 13.5, 13, 12.5],
  [Period.evening]: [18.5, 19, 19.5, 20, 20.5, 21, 21.5, 18, 22],
};

export const ZHANG_XIAO_LONG: ReadonlyMap<'mother', never> = new Map<'mother', never>();

export const GROUP_MAP: ReadonlyMap<Group, GroupDisplay> = new Map([
  [Group.web, 'Web'],
  [Group.lab, 'Lab'],
  [Group.ai, 'AI'],
  [Group.game, 'Game'],
  [Group.android, 'Android'],
  [Group.ios, 'iOS'],
  [Group.design, 'Design'],
  [Group.pm, 'PM'],
]);

export const STEP_SHORT_MAP: ReadonlyMap<Step, string> = new Map([
  [Step.报名, '报名'],
  [Step.笔试, '笔试'],
  [Step.组面时间选择, '组面时间选择'],
  [Step.组面, '组面'],
  [Step.熬测, '熬测'],
  [Step.群面时间选择, '群面时间选择'],
  [Step.群面, '群面'],
  [Step.通过, '通过'],
]);

export const STEP_TYPE_MAP: ReadonlyMap<StepType, string> = new Map([
  [StepType.all, '全部'],
  [StepType.groupInterview, '组面'],
  [StepType.teamInterview, '群面'],
]);

export const PERIOD_MAP: ReadonlyMap<Period, string> = new Map([
  [Period.morning, '上午'],
  [Period.afternoon, '下午'],
  [Period.evening, '晚上'],
]);

export type GroupDisplay = typeof GROUPS[number];
export const GROUPS = ['Web', 'Lab', 'AI', 'Game', 'Android', 'iOS', 'Design', 'PM'] as const;
export const GROUPS_ = GROUPS.map((group) => group.toLowerCase()) as Group[];
export const RANKS = ['暂无', '前10%', '前25%', '前50%', '前100%'] as const;
export const GRADES = ['大一', '大二', '大三', '大四', '研究生'] as const;
export const GENDERS = ['其他', '男', '女'] as const;

export const Departments: Readonly<Record<string, string[]>> = {
  电子信息与通信学院: [
    '通信工程',
    '电子信息类',
    '电子信息工程（卓越）',
    '电子信息工程（提高班）',
    '电磁场与无线技术',
    '电子信息工程',
    '通信工程（中英班）',
    '电子信息工程（中英班）',
  ],
  基础医学院: [
    '医学实验技术',
    '临床医学（八年）',
    '临床医学',
    '临床医学（中德实验班）',
    '医学影像学',
    '中西医临床医学',
    '口腔医学',
    '医学检验技术',
  ],
  土木工程与力学学院: ['土木工程', '土木类', '工程管理', '工程力学', '交通工程', '土木工程（卓越）'],
  计算机科学与技术学院: [
    '计算机类',
    '计算机科学与技术',
    '物联网工程',
    '计算机科学与技术（卓越）',
    '信息安全',
    '计算机科学与技术（交流）',
    '计算机科学与技术（ACM）',
  ],
  光学与电子信息学院: [
    '光电信息科学与工程（卓越）',
    '光电信息科学与工程',
    '电子信息类',
    '光电信息科学与工程（中法）',
    '集成电路设计与集成系统',
    '电子科学与技术',
    '电子科学与技术（中英班）',
    '光电信息科学与工程（王大珩）',
    '微电子科学与工程',
    '集成电路与集成系统（卓越）',
  ],
  生命科学与技术学院: [
    '生物医学工程类',
    '生物医学工程（卓越）',
    '护理学',
    '生物信息学（基地）',
    '临床医学',
    '生物制药',
    '生物医学工程',
    '生物药学（基地）',
    '医学检验技术',
    '生物信息学',
    '药学创新实验班',
    '生物科学（基地）',
    '预防医学',
    '生物技术',
    '生物技术（实验班）',
    '医学影像学',
    '生物科学（拔尖）',
    '公共事业管理',
    '儿科学',
    '中西医临床医学',
    '药学',
    '信息管理与信息系统',
    '法医学',
    '生物科学类',
    '临床医学（中德）',
    '医学实验技术',
    '口腔医学',
  ],
  电气与电子工程学院: ['电气工程及其自动化', '电气工程及其自动化（中英）', '电气工程及其自动化（卓越）'],
  机械科学与工程学院: [
    '机械类',
    '工业工程',
    '机械设计制造及其自动化',
    '测控技术与仪器',
    '产品设计创新班',
    '产品设计',
    '机械设计制造及其自动化（卓越）',
    '机械设计制造及其自动化（中英）',
  ],
  第二临床学院: ['临床医学', '临床医学（八年）', '口腔医学', '中西医临床医学', '临床医学（中德）'],
  管理学院: [
    '信息管理与信息系统',
    '财政学',
    '会计学',
    '工商管理类',
    '工商管理',
    '财务管理',
    '物流管理',
    '管理科学与工程类',
    '管理学创新实验班',
    '市场营销',
  ],
  新闻与信息传播学院: [
    '新闻传播学类',
    '新闻学',
    '传播学',
    '播音与主持艺术',
    '广告学',
    '广播电视学',
    '新闻学(新闻评论)',
  ],
  外国语学院: ['英语', '翻译', '德语', '英语+国际商务', '法语', '日语'],
  能源与动力工程学院: [
    '能源与动力工程',
    '新能源科学与工程',
    '能源动力类',
    '能源与动力工程（卓越）',
    '核工程与核技术',
    '能源与动力工程（中英）',
  ],
  软件学院: ['软件工程', '数字媒体技术'],
  环境科学与工程学院: [
    '建筑环境与能源应用工程',
    '环境工程',
    '环境科学与工程类',
    '给排水科学与工程（卓越）',
    '给排水科学与工程',
  ],
  建筑与城市规划学院: [
    '城乡规划',
    '环境设计（环艺）',
    '建筑学',
    '风景园林',
    '环境设计（数码）',
    '景观学',
    '数字媒体艺术',
  ],
  护理学院: ['护理学'],
  船舶与海洋工程学院: ['海洋工程类', '船舶与海洋工程', '轮机工程', '船舶与海洋工程（卓越）'],
  自动化学院: [
    '自动化（卓越）',
    '自动化（国际交流）',
    '自动化',
    '物流管理',
    '自动化（创新实验班）',
    '测控技术与仪器',
    '自动化类',
  ],
  材料科学与工程学院: [
    '材料成型及控制工程',
    '材料成型及控制工程（卓越）',
    '材料类',
    '功能材料',
    '电子封装技术',
    '材料科学与工程',
  ],
  化学与化工学院: ['化学', '化学类', '应用化学', '临床医学（八年）', '重庆医科大学', '化学（拔尖）', '化学工程与工艺'],
  物理学院: [
    '物理学（4+4）',
    '物理学（天眷英才班）',
    '应用物理学',
    '物理学（严济慈）',
    '物理学类',
    '物理学（拔尖）',
    '物理学',
  ],
  航空航天学院: ['飞行器设计与工程', '飞行器设计与工程（国际）'],
  第一临床学院: ['临床医学', '医学检验技术', '医学影像学', '临床医学（八年）'],
  经济学院: [
    '国际商务+英语',
    '金融学类',
    '经济学类',
    '经济学',
    '经济学（创新实验）',
    '国际经济与贸易',
    '金融学',
    '金融工程',
    '经济统计学',
    '金融学（国际交流）',
  ],
  社会学院: ['社会学', '社会学类', '社会工作'],
  中国语言文学系: ['汉语言文学', '汉语国际教育', '中国语言文学类'],
  药学院: ['生物药学（基地）', '药学', '药学创新实验班'],
  公共管理学院: ['公共管理类', '公共事业管理', '政治学与行政学'],
  法学院: ['法学'],
  水电与数字化工程学院: ['水利水电工程'],
  医药卫生管理学院: ['信息管理与信息系统', '公共事业管理'],
  公共卫生学院: ['预防医学', '预防医学（五年）'],
  哲学系: ['哲学'],
  数学与统计学院: ['数学类', '数学与应用数学（拔尖）', '信息与计算科学', '统计学', '数学与应用数学'],
  工程科学学院: ['生物医学工程', '电子科学与技术', '光电信息科学与工程'],
  法医学系: ['法医学'],
  网络空间安全学院: ['信息安全', '网络空间安全'],
};
